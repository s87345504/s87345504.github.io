import { L as Lt } from "./utils-dYOJfn2v.js";
(function polyfill() {
...
const useGradientStore = /* @__PURE__ */ defineStore("GradientOptions", () => {
  ...
  if (localStorage.getItem("gradientOptions")) {
    options.value = JSON.parse(localStorage.getItem("gradientOptions"));
  }
  return {
    options
  };
...
const _sfc_main$2 = {
  ...
  props: {
    colors: {
      type: Array,
      default: ["#4098DB", "#ECF3FC", "#C1EBFB", "#A9E0F8", "#5FD2FA", "#4CB0ED"]
    },
    loop: {
      type: Boolean,
      default: true
  ...
  setup(__props) {
    ...
    onMounted(() => {
      ...
      subscribe = gradientStore.$subscribe((_, state) => {
        localStorage.setItem("gradientOptions", JSON.stringify(state.options));
        Object.entries(state.options).forEach(([key, value]) => {
          colorbg.update(key, value);
        });
      });
      gradientStore.$patch((state) => {
        state.options.scale = state.options.scale || 0.15;
      });
    });
    onUnmounted(() => {
      colorbg.destroy();
...
const _sfc_main$1 = {
  ...
  setup(__props, { emit: __emit }) {
    ...
    const list_content = ref(/* @__PURE__ */ new Set());
    const list_count = computed(() => {
      return list_content.value.size;
    });
    onMounted(() => {
      loadhistory();
    });
      ...
      return openBlock(), createElementBlock("div", _hoisted_1, [
        createBaseVNode("form", {
          ...
          withDirectives(createBaseVNode("input", {
            class: normalizeClass({ "open-list": list_show.value }),
            type: "search",
            name: "search",
